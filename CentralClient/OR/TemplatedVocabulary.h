...
void TemplatedVocabulary<TDescriptor,F>::createWords()
{
  ...
  for(++nit; nit != m_nodes.end(); ++nit)
  {
    if(nit->isLeaf())
    {
      ...
      m_words.push_back( &(*nit) );	//(ZJZ) m_words stores the pointers to leaves
    }
  }
}
  ...
  else if(m_weighting == IDF || m_weighting == TF_IDF)
  {
    ...
    for(mit = training_features.begin(); mit != training_features.end(); ++mit)
    {
      ...
      for(fit = mit->begin(); fit < mit->end(); ++fit)
      {
        ...
        if(!counted[word_id]) //(ZJZ) one image enters once at most
        {
          Ni[word_id]++;
...
// --------------------------------------------------------------------------
// transform the sift descriptor to bag of words vector.
template<class TDescriptor, class F>
  ...
  const std::vector<TDescriptor>& features, BowVector &v) const
{
  ...
  if(m_weighting == TF || m_weighting == TF_IDF)
  {
    for(fit = features.begin(); fit < features.end(); ++fit)
    {
      ...
      transform(*fit, id, w);  //transform the sift descriptor into word with word value
...
// --------------------------------------------------------------------------
////transform the sift descriptor into word with word value
template<class TDescriptor, class F>
...
{ 
  ...
  {
    ++current_level;
    nodes = m_nodes[final_id].children;  //m_nodes stored from the database, nodes is a array containing all the children
    final_id = nodes[0];
  ...
  weight = m_nodes[final_id].weight; //the weights in the vocabulary
}
...
void TemplatedVocabulary<TDescriptor,F>::load(cv::FileStorage &f,
  const std::string &name) // default is "Vocabulary" (ZJZ)
{
  m_words.clear();
  m_nodes.clear();
